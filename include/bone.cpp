#/*********************************************************************/
//   bone.cpp -- Implementation of Inverse Kinematics using CCD	   *
...
void Bone::checkConstraints(float x, float y, float z) {
    if (x < constraint[0].x || x > constraint[1].x ||
        ...
        printf("%f, %f, %f\n(%f,%f), (%f,%f), (%f,%f)\n", x, y, z, constraint[0].x, constraint[1].x, constraint[0].y,
               constraint[1].y, constraint[0].z, constraint[1].z);
...
unsigned long long Bone::id() {
    if (parent == NULL)
        ...
        return parent->id() * 10 +
               (std::find(parent->bones.begin(), parent->bones.end(), this) - parent->bones.begin()) + 1;
}
